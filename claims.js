...
async function uploadSlackEvidence(token, channel, claim, file) {
  if (!file?.base64 || !file?.file_name) return { sent: false, skipped: true }
  const buffer = Buffer.from(file.base64, 'base64')
  const urlResponse = await fetch('https://slack.com/api/files.getUploadURLExternal', {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({ filename: file.file_name, length: String(buffer.length) }),
  })
  const uploadInfo = await urlResponse.json()
  if (!urlResponse.ok || !uploadInfo.ok) throw new Error(uploadInfo.error || 'Slack evidence upload URL failed')

  const uploadResponse = await fetch(uploadInfo.upload_url, {
    method: 'POST',
    headers: { 'Content-Type': file.file_type || 'application/octet-stream' },
    body: buffer,
  })
  if (!uploadResponse.ok) throw new Error(`Slack evidence upload failed (HTTP ${uploadResponse.status})`)
  ...
  const completeResponse = await fetch('https://slack.com/api/files.completeUploadExternal', {
    ...
    body: JSON.stringify({
      channel_id: channel,
      initial_comment: `:paperclip: *Evidence for ${slackSafe(claim.claim_number)}*`,
      files: [{ id: uploadInfo.file_id, title: file.file_name }],
    }),
  })
  const complete = await completeResponse.json()
  if (!completeResponse.ok || !complete.ok) throw new Error(complete.error || 'Slack evidence completion failed')
  return { sent: true, fileId: uploadInfo.file_id }
}

async function sendSlackClaim(claim, items, evidence = []) {
  ...
  for (const file of (evidence || []).slice(0, 8)) evidenceResults.push(await uploadSlackEvidence(token, channel, claim, file))
  return { sent: true, ts: data.ts, evidenceSent: evidenceResults.filter(result => result.sent).length }
}
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      try {
        slack = await sendSlackClaim(saved, items, evidence)
      } catch (slackError) {
    ...
    if (req.method === 'GET') {
      ...
      if (claimId) {
        const [{ data: claim, error: claimError }, { data: items, error: itemsError }, { data: history, error: historyError }] = await Promise.all([
        ...
        return res.status(200).json({ claim, items: items || [], evidence: [], history: history || [] })
      }
